 ...
 *  - `queueOptions`, which is passed to channel.assertQueue method;
 *  - `exchangeOptions`, which is passed to channel.assertExchange method;
 *
...
AMQPLibAscoltatore.prototype._startConn = function () {
  ...
  if (this._client_conn === undefined) {
    ...
    var queueOptions = this._opts.queueOptions || {};
    var exchangeOptions = this._opts.exchangeOptions || {};
    ...
    steed.series([
      ...
      function(callback){
        ...
        that._channel.assertQueue(that._queue, queueOptions, wrap(callback));
      },

      function (callback){
        debug('queue created');
        that._channel.assertExchange(that._opts.exchange, 'topic', exchangeOptions, wrap(callback));
      },
